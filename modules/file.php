...
function wpcf7_file_messages( $messages ) {
	return array_merge( $messages, array(
		'upload_failed' => array(
			'description' => __( "Uploading a file fails for any reason", 'valhalla-contact-form' ),
			'default' => __( "There was an unknown error uploading the file.", 'valhalla-contact-form' )
		),

		'upload_file_type_invalid' => array(
			'description' => __( "Uploaded file is not allowed for file type", 'valhalla-contact-form' ),
			'default' => __( "You are not allowed to upload files of this type.", 'valhalla-contact-form' )
		),

		'upload_file_too_large' => array(
			'description' => __( "Uploaded file is too large", 'valhalla-contact-form' ),
			'default' => __( "The file is too big.", 'valhalla-contact-form' )
		),

		'upload_failed_php_error' => array(
			'description' => __( "Uploading a file fails for PHP error", 'valhalla-contact-form' ),
			'default' => __( "There was an error uploading the file.", 'valhalla-contact-form' )
		)
	) );
}
...
function wpcf7_add_tag_generator_file() {
	...
	$tag_generator->add( 'file', __( 'file', 'valhalla-contact-form' ),
...
function wpcf7_tag_generator_file( $contact_form, $args = '' ) {
	...
	$description = __( "Generate a file uploading field. For more details, see %s.", 'valhalla-contact-form' );
...
<tbody>
	...
	<tr>
	<th scope="row"><label for="<?php echo esc_attr( $args['content'] . '-limit' ); ?>"><?php echo esc_html( __( "File size limit (bytes)", 'valhalla-contact-form' ) ); ?></label></th>
	<td><input type="text" name="limit" class="filesize oneline option" id="<?php echo esc_attr( $args['content'] . '-limit' ); ?>" /></td>
	</tr>

	<tr>
	<th scope="row"><label for="<?php echo esc_attr( $args['content'] . '-filetypes' ); ?>"><?php echo esc_html( __( 'Acceptable file types', 'valhalla-contact-form' ) ); ?></label></th>
...
<div class="insert-box">
	...
	<p class="description mail-tag"><label for="<?php echo esc_attr( $args['content'] . '-mailtag' ); ?>"><?php echo sprintf( esc_html( __( "To attach the file uploaded through this field to mail, you need to insert the corresponding mail-tag (%s) into the File Attachments field on the Mail tab.", 'valhalla-contact-form' ) ), '<strong><span class="mail-tag"></span></strong>' ); ?><input type="text" class="mail-tag code hidden" readonly="readonly" id="<?php echo esc_attr( $args['content'] . '-mailtag' ); ?>" /></label></p>
</div>
<?php
}
...
function wpcf7_file_display_warning_message( $page, $action, $object ) {
	...
	or ! wp_is_writable( $uploads_dir ) ) {
		$message = sprintf( __( 'This contact form contains file uploading fields, but the temporary folder for the files (%s) does not exist or is not writable. You can create the folder or change its permission manually.', 'valhalla-contact-form' ), $uploads_dir );
