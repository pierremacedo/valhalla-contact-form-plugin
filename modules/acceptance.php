...
function wpcf7_acceptance_form_tag_handler( $tag ) {
	...
	if ( $content ) {
		$html = sprintf(
			'<span class="wpcf7-list-item form-check"><input %1$s class="form-check-input" /><span class="vhcf-check-label-after">%2$s</span></span>',
			$item_atts, $content );
	} else {
		$html = sprintf(
			'<span class="wpcf7-list-item"><input %1$s class="form-control-input" /></span>',
			$item_atts );
	}
...
function wpcf7_acceptance_mail_tag( $replaced, $submitted, $html, $mail_tag ) {
	...
	if ( ! empty( $submitted ) ) {
		$replaced = __( 'Consented', 'valhalla-contact-form' );
	} else {
		$replaced = __( 'Not consented', 'valhalla-contact-form' );
	}
...
function wpcf7_add_tag_generator_acceptance() {
	...
	$tag_generator->add( 'acceptance', __( 'acceptance', 'valhalla-contact-form' ),
...
function wpcf7_tag_generator_acceptance( $contact_form, $args = '' ) {
	...
	$description = __( "Generate an acceptance checkbox. For more details, see %s.", 'valhalla-contact-form' );
...
<tbody>
	...
	<tr>
	<th scope="row"><label for="<?php echo esc_attr( $args['content'] . '-content' ); ?>"><?php echo esc_html( __( 'Condition', 'valhalla-contact-form' ) ); ?></label></th>
	...
	<td>
		...
		<label><input type="checkbox" name="optional" class="option" checked="checked" /> <?php echo esc_html( __( 'Make this checkbox optional', 'valhalla-contact-form' ) ); ?></label>
		</fieldset>
	</td>
	</tr>
