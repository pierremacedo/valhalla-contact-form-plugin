...
function wpcf7_date_messages( $messages ) {
	return array_merge( $messages, array(
		'invalid_date' => array(
			'description' => __( "Date format that the sender entered is invalid", 'valhalla-contact-form' ),
			'default' => __( "The date format is incorrect.", 'valhalla-contact-form' )
		),

		'date_too_early' => array(
			'description' => __( "Date is earlier than minimum limit", 'valhalla-contact-form' ),
			'default' => __( "The date is before the earliest one allowed.", 'valhalla-contact-form' )
		),

		'date_too_late' => array(
			'description' => __( "Date is later than maximum limit", 'valhalla-contact-form' ),
			'default' => __( "The date is after the latest one allowed.", 'valhalla-contact-form' )
		),
	) );
}
...
function wpcf7_add_tag_generator_date() {
	...
	$tag_generator->add( 'date', __( 'date', 'valhalla-contact-form' ),
...
function wpcf7_tag_generator_date( $contact_form, $args = '' ) {
	...
	$description = __( "Generate a date field. For more details, see %s.", 'valhalla-contact-form' );
