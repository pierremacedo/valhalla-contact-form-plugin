...
function wpcf7_admin_menu() {
	...
	add_menu_page( __( 'Contact Form 7', 'valhalla-contact-form' ),
		__( 'Valhalla Contact', 'valhalla-contact-form' )
	...
	$edit = add_submenu_page( 'wpcf7',
		__( 'Edit Contact Form', 'valhalla-contact-form' ),
		__( 'Contact Forms', 'valhalla-contact-form' )
	...
	$addnew = add_submenu_page( 'wpcf7',
		__( 'Add New Contact Form', 'valhalla-contact-form' ),
	...
	if ( $integration->service_exists() ) {
		$integration = add_submenu_page( 'wpcf7',
			__( 'Integration with reCAPTCHA', 'valhalla-contact-form' ),
			__( 'Captcha', 'valhalla-contact-form' )
...
function wpcf7_admin_enqueue_scripts( $hook_suffix ) {
	...
	wp_enqueue_style( 'valhalla-contact-form-admin',
		wpcf7_plugin_url( 'admin/css/styles.css' ),
		array(), WPCF7_VERSION, 'all'
	);

	if ( wpcf7_is_rtl() ) {
		wp_enqueue_style( 'valhalla-contact-form-admin-rtl',
	...
	$args = array(
		'apiSettings' => array(
			'root' => esc_url_raw( rest_url( 'valhalla-contact-form/v1' ) ),
			'namespace' => 'valhalla-contact-form/v1',
		...
		'configValidator' => array(
			'errors' => array(),
			'oneError' => __( '1 configuration error detected', 'valhalla-contact-form' ),
			'manyErrors' => __( '%d configuration errors detected', 'valhalla-contact-form' ),
			'oneErrorInTab' => __( '1 configuration error detected in this tab panel', 'valhalla-contact-form' ),
			'manyErrorsInTab' => __( '%d configuration errors detected in this tab panel', 'valhalla-contact-form' ),
			'docUrl' => WPCF7_ConfigValidator::get_doc_link(),
			/* translators: screen reader text */
			'iconAlt' => __( '(configuration error)', 'valhalla-contact-form' ),
		),
	);
...
function wpcf7_dark_mode_support( $user_id ) {
	wp_enqueue_style( 'valhalla-contact-form-admin-dark-mode',
		wpcf7_plugin_url( 'admin/css/styles-dark-mode.css' ),
		array( 'valhalla-contact-form-admin' ), WPCF7_VERSION, 'screen' );
}
...
function wpcf7_load_contact_form_admin() {
	...
	if ( 'save' == $action ) {
		...
		if ( ! current_user_can( 'wpcf7_edit_contact_form', $id ) ) {
			wp_die( __( 'You are not allowed to edit this item.', 'valhalla-contact-form' ) );
		}
        ...
        $args['google_maps'] = isset( $_POST['wpcf7-google-maps'] )
			? $_POST['wpcf7-google-maps'] : array();
        
        $args['custom_style'] = isset( $_POST['wpcf7-custom-style'] )
			? $_POST['wpcf7-custom-style'] : '';
	...
	if ( 'delete' == $action ) {
		...
		foreach ( $posts as $post ) {
			...
			if ( ! current_user_can( 'wpcf7_delete_contact_form', $post->id() ) ) {
				wp_die( __( 'You are not allowed to delete this item.', 'valhalla-contact-form' ) );
			}

			if ( ! $post->delete() ) {
				wp_die( __( 'Error in deleting.', 'valhalla-contact-form' ) );
			}
...
<h1 class="wp-heading-inline"><?php
	echo esc_html( __( 'Contact Forms', 'valhalla-contact-form' ) );
?></h1>
...
<?php
	...
	if ( ! empty( $_REQUEST['s'] ) ) {
		echo sprintf( '<span class="subtitle">'
			...
			. __( 'Search results for &#8220;%s&#8221;', 'valhalla-contact-form' )
...
<form method="get" action="">
	...
	<?php $list_table->search_box( __( 'Search Contact Forms', 'valhalla-contact-form' ), 'wpcf7-contact' ); ?>
...
<h1><?php echo esc_html( __( 'Integration with reCAPTCHA', 'valhalla-contact-form' ) ); ?></h1>
...
function wpcf7_admin_updated_message( $page, $action, $object ) {
	...
	if ( 'created' == $_REQUEST['message'] ) {
		$updated_message = __( "Contact form created.", 'valhalla-contact-form' );
	} elseif ( 'saved' == $_REQUEST['message'] ) {
		$updated_message = __( "Contact form saved.", 'valhalla-contact-form' );
	} elseif ( 'deleted' == $_REQUEST['message'] ) {
		$updated_message = __( "Contact form deleted.", 'valhalla-contact-form' );
	}
		...
		if ( $count_invalid ) {
			$updated_message = sprintf(
				...
				_n(
					...
					$count_invalid, 'valhalla-contact-form' ),
			...
			$updated_message = __( "Configuration validation completed. No invalid contact form was found.", 'valhalla-contact-form' );
...
function wpcf7_plugin_action_links( $links, $file ) {
	...
	$settings_link = wpcf7_link(
		...
		__( 'Settings', 'valhalla-contact-form' )
	);
...
<p><?php
	...
	echo sprintf( __( '<strong>Valhalla Contact Form requires WordPress %2$s or higher.</strong> Please <a href="%3$s">update WordPress</a> first.', 'valhalla-contact-form' ), WPCF7_VERSION, WPCF7_REQUIRED_WP_VERSION, admin_url( 'update-core.php' ) );
?></p>
</div>
<?php
}
