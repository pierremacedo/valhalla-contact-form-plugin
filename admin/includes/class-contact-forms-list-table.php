...
class WPCF7_Contact_Form_List_Table extends WP_List_Table {
	...
	public static function define_columns() {
		$columns = array(
			...
			'title' => __( 'Title', 'valhalla-contact-form' ),
			'shortcode' => __( 'Shortcode', 'valhalla-contact-form' ),
			'author' => __( 'Author', 'valhalla-contact-form' ),
			'date' => __( 'Date', 'valhalla-contact-form' ),
		);
	...
	protected function get_bulk_actions() {
		$actions = array(
			'delete' => __( 'Delete', 'valhalla-contact-form' ),
		);
	...
	public function column_title( $item ) {
		...
		$output = sprintf(
			...
			esc_attr( sprintf( __( 'Edit &#8220;%s&#8221;', 'valhalla-contact-form' ),
				$item->title() ) ),
			esc_html( $item->title() )
		);
			...
			if ( $count_errors = $config_validator->count_errors() ) {
				$error_notice = sprintf(
					...
					_n(
						...
						$count_errors, 'valhalla-contact-form' ),
	...
	protected function handle_row_actions( $item, $column_name, $primary ) {
		...
		$actions = array(
			'edit' => wpcf7_link( $edit_link, __( 'Edit', 'valhalla-contact-form' ) ),
		);
		...
		if ( current_user_can( 'wpcf7_edit_contact_form', $item->id() ) ) {
			...
			$actions = array_merge( $actions, array(
				'copy' => wpcf7_link( $copy_link, __( 'Duplicate', 'valhalla-contact-form' ) ),
			) );
		}
	...
	public function column_date( $item ) {
		...
		$t_time = mysql2date( __( 'Y/m/d g:i:s A', 'valhalla-contact-form' ),
			$post->post_date, true );
		...
		if ( $time_diff > 0 and $time_diff < 24*60*60 ) {
			$h_time = sprintf(
				...
				__( '%s ago', 'valhalla-contact-form' ),
				human_time_diff( $time )
			);
		} else {
			$h_time = mysql2date( __( 'Y/m/d', 'valhalla-contact-form' ), $m_time );
		}
