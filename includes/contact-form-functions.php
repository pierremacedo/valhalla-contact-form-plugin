...
function wpcf7_form_controls_class( $type, $default = '' ) {
    ...
    if (($typebase=='text') ||($typebase=='email') ||($typebase=='url') ||($typebase=='tel') ||($typebase=='number') ||($typebase=='date') ||($typebase=='textarea') ||($typebase=='select') ||($typebase=='quiz')) {
  ...
  if ($typebase=='file') {
    ...
    if ($typebase=='submit') {
    $classes[] = 'btn btn-success btn-send vhcf-submit-button';
  }
...
function wpcf7_contact_form_tag_func( $atts, $content = null, $code = '' ) {
	if ( is_feed() ) {
		return '[valhalla-contact-form]';
	}

	if ( 'valhalla-contact-form' == $code ) {
		$atts = shortcode_atts(
			array(
				'id' => 0,
				'title' => '',
	...
	if ( ! $contact_form ) {
		return '[valhalla-contact-form 404 "Not Found"]';
	}
...
function wpcf7_save_contact_form( $args = '', $context = 'save' ) {
    ...
    $properties['google_maps'] = wpcf7_sanitize_google_maps(
		$args['google_maps'], $properties['google_maps'] );
  
    $properties['custom_style'] = wpcf7_sanitize_custom_style(
		$args['custom_style'], $properties['custom_style'] );
...
function wpcf7_sanitize_google_maps( $input, $defaults = array() ) {
	$defaults = wp_parse_args( $defaults, array(
	...
	$output = array();
	$output['class'] = trim( $input['class'] );
	$output['location'] = trim( $input['location'] );

	return $output;
}

function wpcf7_sanitize_custom_style( $input, $default = '' ) {
	if ( null === $input ) {
		return $default;
	}
