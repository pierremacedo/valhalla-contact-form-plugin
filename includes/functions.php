...
function wpcf7_deprecated_function( $function, $version, $replacement ) {
	...
	if ( WP_DEBUG and $trigger_error ) {
		if ( function_exists( '__' ) ) {
			trigger_error( sprintf( __( '%1$s is <strong>deprecated</strong> since Contact Form 7 version %2$s! Use %3$s instead.', 'valhalla-contact-form' ), $function, $version, $replacement ) );
		} else {
...
function wpcf7_log_remote_request( $url, $request, $response ) {
	$log = sprintf(
		...
		__( 'HTTP Response: %1$s %2$s from %3$s', 'valhalla-contact-form' ),
...
function vhcf_add_google_fonts() {
 
wp_enqueue_style( 'vhcf-google-fonts', 'https://fonts.googleapis.com/css?family=Source+Sans+Pro', false ); 
}
 
add_action( 'wp_enqueue_scripts', 'vhcf_add_google_fonts' );
...
function vhcf_name_field_label(){
    $contact_form = WPCF7_ContactForm::get_current();
    $name_field =  $contact_form->message( 'name_field_label' );
    return $name_field;
}
add_shortcode( 'vhcf_name_field_label', 'vhcf_name_field_label' );

function vhcf_email_field_label(){
    $contact_form = WPCF7_ContactForm::get_current();
    $email_field =  $contact_form->message( 'email_field_label' );
    return $email_field;
}
add_shortcode( 'vhcf_email_field_label', 'vhcf_email_field_label' );

function vhcf_subject_field_label(){
    $contact_form = WPCF7_ContactForm::get_current();
    $subject_field =  $contact_form->message( 'subject_field_label' );
    return $subject_field;
}
add_shortcode( 'vhcf_subject_field_label', 'vhcf_subject_field_label' );

function vhcf_phone_field_label(){
    $contact_form = WPCF7_ContactForm::get_current();
    $phone_field =  $contact_form->message( 'phone_field_label' );
    return $phone_field;
}
add_shortcode( 'vhcf_phone_field_label', 'vhcf_phone_field_label' );

function vhcf_message_field_label(){
    $contact_form = WPCF7_ContactForm::get_current();
    $message_field =  $contact_form->message( 'message_field_label' );
    return $message_field;
}
add_shortcode( 'vhcf_message_field_label', 'vhcf_message_field_label' );

function vhcf_google_maps_location(){
    $contact_form = WPCF7_ContactForm::get_current();
    $google_maps =  $contact_form->prop('google_maps');
    $location = $google_maps['location'];
    return $location;
}
add_shortcode( 'vhcf_google_maps_location', 'vhcf_google_maps_location' );

function vhcf_custom_style(){
    $contact_form = WPCF7_ContactForm::get_current();
    $custom_style =  $contact_form->prop( 'custom_style' );
    return $custom_style;
}
add_shortcode( 'vhcf_custom_style', 'vhcf_custom_style' );

/* Add shortcode to enable or disable Google Maps */

function vhcf_google_maps_class(){
    $contact_form = WPCF7_ContactForm::get_current();
    $google_maps =  $contact_form->prop('google_maps');
    $class = $google_maps['class'];
    return $class;
}
add_shortcode( 'vhcf_google_maps_class', 'vhcf_google_maps_class' );