...
function wpcf7_rest_get_contact_forms( WP_REST_Request $request ) {
	if ( ! current_user_can( 'wpcf7_read_contact_forms' ) ) {
		return new WP_Error( 'wpcf7_forbidden',
			__( "You are not allowed to access contact forms.", 'valhalla-contact-form' ),
			array( 'status' => 403 ) );
	}
...
function wpcf7_rest_create_contact_form( WP_REST_Request $request ) {
	...
	if ( $id ) {
		return new WP_Error( 'wpcf7_post_exists',
			__( "Cannot create existing contact form.", 'valhalla-contact-form' ),
			array( 'status' => 400 ) );
	}

	if ( ! current_user_can( 'wpcf7_edit_contact_forms' ) ) {
		return new WP_Error( 'wpcf7_forbidden',
			__( "You are not allowed to create a contact form.", 'valhalla-contact-form' ),
			array( 'status' => 403 ) );
	}
	...
	if ( ! $item ) {
		return new WP_Error( 'wpcf7_cannot_save',
			__( "There was an error saving the contact form.", 'valhalla-contact-form' ),
			array( 'status' => 500 ) );
	}
...
function wpcf7_rest_get_contact_form( WP_REST_Request $request ) {
	...
	if ( ! $item ) {
		return new WP_Error( 'wpcf7_not_found',
			__( "The requested contact form was not found.", 'valhalla-contact-form' ),
			array( 'status' => 404 ) );
	}

	if ( ! current_user_can( 'wpcf7_edit_contact_form', $id ) ) {
		return new WP_Error( 'wpcf7_forbidden',
			__( "You are not allowed to access the requested contact form.", 'valhalla-contact-form' ),
			array( 'status' => 403 ) );
	}
...
function wpcf7_rest_delete_contact_form( WP_REST_Request $request ) {
	...
	if ( ! $result ) {
		return new WP_Error( 'wpcf7_cannot_delete',
			__( "There was an error deleting the contact form.", 'valhalla-contact-form' ),
			array( 'status' => 500 ) );
	}
