...
class WPCF7_ContactForm {
	...
	public static function register_post_type() {
		register_post_type( self::post_type, array(
			'labels' => array(
				'name' => __( 'Contact Forms', 'valhalla-contact-form' ),
				'singular_name' => __( 'Contact Form', 'valhalla-contact-form' ),
			),
			'rewrite' => false,
			'query_var' => false,
			'public' => false,
	...
	public static function get_template( $args = '' ) {
		...
		if ( $locale ) {
			$mo_orig = $l10n['valhalla-contact-form'];
			wpcf7_load_textdomain( $locale );
		}

		self::$current = $contact_form = new self;
		$contact_form->title =
			( $title ? $title : __( 'Untitled', 'valhalla-contact-form' ) );
		...
		if ( isset( $mo_orig ) ) {
			$l10n['valhalla-contact-form'] = $mo_orig;
		}
	...
	public function __get( $name ) {
		$message = __( '<code>%1$s</code> property of a <code>WPCF7_ContactForm</code> object is <strong>no longer accessible</strong>. Use <code>%2$s</code> method instead.', 'valhalla-contact-form' );
	...
	public function set_title( $title ) {
		...
		if ( '' === $title ) {
			$title = __( 'Untitled', 'valhalla-contact-form' );
		}
	...
	public function form_html( $args = '' ) {
        ...
        $gmaps_disable = '[vhcf_google_maps_class]';
        $gmaps_url = '[vhcf_google_maps_location]';
        $custom_style = '[vhcf_custom_style]';
		...
		$html .= '<div id="vhcf-container" class="container-fluid vhcf-form-width"><div class="row"><div '.do_shortcode( $gmaps_disable ).'class="col-md">';
        $html .= '<div class="mapouter"><div class="gmap_canvas"><iframe max-width="500px" width="100%" height="100%" id="gmap_canvas" src="https://maps.google.com/maps?q='.do_shortcode( $gmaps_url ).'&t=&z=13&ie=UTF8&iwloc=&output=embed" frameborder="0" scrolling="no" marginheight="0" marginwidth="0"></iframe>';
        $html .= '</div></div></div><div id="vhcf-form-div" class="col-md vhcf-bg"><style>'.do_shortcode( $custom_style ).'</style>';
	...
	public function shortcode( $args = '' ) {
		...
		if ( $args['use_old_format'] ) {
			...
			$shortcode = sprintf( '[valhalla-contact-form id="%1$d" title="%2$s"]',
				$this->id, $title );
		}
