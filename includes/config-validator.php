	...
	public function get_default_message( $code ) {
		switch ( $code ) {
			case self::error_maybe_empty:
				return __( "There is a possible empty field.", 'valhalla-contact-form' );
			case self::error_invalid_mailbox_syntax:
				return __( "Invalid mailbox syntax is used.", 'valhalla-contact-form' );
			case self::error_email_not_in_site_domain:
				return __( "Sender email address does not belong to the site domain.", 'valhalla-contact-form' );
			case self::error_html_in_message:
				return __( "HTML tags are used in a message.", 'valhalla-contact-form' );
			case self::error_multiple_controls_in_label:
				return __( "Multiple form controls are in a single label element.", 'valhalla-contact-form' );
			case self::error_invalid_mail_header:
				return __( "There are invalid mail header fields.", 'valhalla-contact-form' );
			case self::error_deprecated_settings:
				return __( "Deprecated settings are used.", 'valhalla-contact-form' );
			default:
				return '';
		}
	}
			...
			return $this->add_error( $section,
				...
				array(
					'message' =>
						...
						__( "Unavailable names (%names%) are used for form controls.", 'valhalla-contact-form' ),
			...
			if ( ! preg_match( '/^([0-9A-Za-z-]+):(.*)$/', $header, $matches ) ) {
				...
				if ( in_array( strtolower( $header_name ), $mailbox_header_types ) ) {
					$this->detect_invalid_mailbox_syntax(
						...
						$header_value, array(
							'message' =>
								__( "Invalid mailbox syntax is used in the %name% field.", 'valhalla-contact-form' ),
	...
	public function detect_file_not_found( $section, $content ) {
		...
		or ! is_file( $path ) ) {
			return $this->add_error( $section,
				...
				array(
					'message' =>
						__( "Attachment file does not exist at %path%.", 'valhalla-contact-form' ),
	...
	public function detect_file_not_in_content_dir( $section, $content ) {
		...
		if ( ! wpcf7_is_file_path_in_content_dir( $path ) ) {
			return $this->add_error( $section,
				...
				array(
					'message' =>
						__( "It is not allowed to use files outside the wp-content directory.", 'valhalla-contact-form' ),
